...
async function fetchStaff(
  ...
  const response = await axios.get(
    `/api/staff?page=${page}&limit=${pageSize}&search=${search}${role ? `&role=${role}` : ""}`,
  )
  return {
    data: response.data,
...
export default function useStaff(page: number, search: string, activeRole: string, pageSize: number) {
  return useQuery({
